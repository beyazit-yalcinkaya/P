...
namespace Plang.Compiler.TypeChecker
{
    public class StatementVisitor : PParserBaseVisitor<IPStmt>
    {
        ...
        public override IPStmt VisitFunctionBody(PParser.FunctionBodyContext context)
        {
            ...
            if (method.Role.HasFlag(FunctionRole.EntryHandler) && machine.States.ToList().Exists(x => x.Entry != null && x.Entry.Equals(method) && x.RTAModule != null))
            {
                Variable isFirstRun = machine.Fields.ToList().Find(x => x.Name.Equals("*isFirstRun") && x.Type.Equals(PrimitiveType.Bool));
                AssignStmt assignStmt = new AssignStmt(context, new VariableAccessExpr(context, isFirstRun), new BoolLiteralExpr(context, true));
                statements.Add(assignStmt);
            }
            else if (method.Role.HasFlag(FunctionRole.RTAModule))
            {
                ...
                List<IPStmt> isFirstRunCheckThenStatements = new List<IPStmt>();
                IPStmt isFirstRunFalseAssignment = new AssignStmt(context, new VariableAccessExpr(context, isFirstRun), new BoolLiteralExpr(context, false));
                isFirstRunCheckThenStatements.Add(isFirstRunFalseAssignment);
                ...
                isFirstRunCheckThenStatements.Add(modeAssignment);
                ...
                isFirstRunCheckThenStatements.Add(controllerCheck);
                isFirstRunCheckThenStatements.Add(new AssignStmt(context, new VariableAccessExpr(context, decisionPeriodCount), new IntLiteralExpr(context, 1)));

                IPExpr isFirstRunCheckCondition = new VariableAccessExpr(context, isFirstRun);
                IPStmt isFirstRunCheckThenBody = new CompoundStmt(context, isFirstRunCheckThenStatements);
                ...
                IPStmt isFirstRunElseBody = new IfStmt(context, checkDecisionPeriod, new CompoundStmt(context, thenStatements), new CompoundStmt(context, elseStatements));
                IPStmt isFirstRunIfStmt = new IfStmt(context, isFirstRunCheckCondition, isFirstRunCheckThenBody, isFirstRunElseBody);

                statements.Add(isFirstRunIfStmt);
            }
